-- `source` einer wiederhergestellten Zahlung bleibt erhalten.
--
-- Gefunden bei der ersten echten Restore-Probe im eigenen Konto (2026-07-29):
-- `new row for relation "dividend_payments" violates check constraint
-- "import_fields_consistency"`.
--
-- Die Bedingung aus 0016 ist eine strikte Aequivalenz:
--
--   (source in ('csv_import','excel_import'))
--     = (import_id is not null and source_row_number is not null
--        and row_fingerprint is not null)
--
-- 0023 setzte `source` fest auf 'restore', uebernahm die drei Importfelder
-- aber unveraendert. Damit war die rechte Seite wahr und die linke falsch:
-- **jede importierte Zahlung** scheiterte. Bei diesem Projekt sind das
-- praktisch alle — die Wiederherstellung war damit weiterhin unbenutzbar,
-- obwohl Format- und Referenzpruefung bereits durchliefen.
--
-- Die Fassung aus 0022 machte es richtig (`coalesce(obj->>'source', ...)`);
-- der Fehler entstand erst beim Neuschreiben der Funktion. Fachlich ist das
-- Bewahren auch das Richtige: Woher eine Zahlung stammt, gehoert zu ihrer
-- Geschichte. Dass sie durch eine Wiederherstellung erneut geschrieben wurde,
-- haelt das Audit Log fest (Herkunft 'restore').
--
-- Der Rumpf entspricht dem aus 0025; geaendert ist ausschliesslich die Zeile
-- fuer `source`.
...
begin
  ...
  select
    ...
    -- `source` bleibt, wie es war. Es fest auf 'restore' zu setzen verletzte
    -- `import_fields_consistency`: Die Bedingung verlangt, dass genau dann eine
    -- Importquelle steht, wenn import_id, source_row_number und
    -- row_fingerprint gesetzt sind. Die drei Felder wurden uebernommen, die
    -- Quelle nicht — jede importierte Zahlung scheiterte daran.
    --
    -- Fachlich ist das Bewahren ohnehin richtig: Woher eine Zahlung
    -- urspruenglich stammt, ist Teil ihrer Geschichte. Dass sie durch eine
    -- Wiederherstellung erneut geschrieben wurde, steht im Audit Log
    -- (Herkunft 'restore', siehe oben).
